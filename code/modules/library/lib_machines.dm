...
/obj/machinery/librarypubliccomp/interact(mob/user)
	usr.set_machine(src)
	...
	switch(screenstate)
		...
		if(1)
			...
			var/savefile/S = server_db.access("/books")
			if(S)
				...
				for(var/dir in S.dir)
					...
					if(category && category != B.category)
						continue
					if(title && !findtext(B.title, title))
						continue
					if(author && !findtext(B.author, author))
						continue
					dat += "<tr><td>[B.author]</td><td>[B.title]</td><td>[B.category]</td><td>[dir]</td></tr>"
			else
				dat += "<tr><td colspan=4><font color='red'>Unable to access database</font></td></tr>"
			dat += "</table><br>"
...
/obj/machinery/librarycomp/interact(mob/user)
	usr.set_machine(src)
	...
	switch(screenstate)
		...
		if(4)
			...
			if(S)
				...
				for(var/dir in S.dir)
					...
					if(!istype(B))
						continue
					dat += "<tr><td>[B.author]</td><td>[B.title]</td><td>[B.category]</td><td><A href='?src=\ref[src];targetid=[dir]'>\[Order\]</A></td></tr>"
			else
				dat += "<tr><td colspan=4><font color='red'>Unable to access database</font></td></tr>"
			dat += "</table>"
		...
		if(5)
			...
			if(!scanner)
				...
				dat += "<TT>Title: </TT>[scanner.cache.title ? scanner.cache.title : "*none*"]<BR>"
				dat += "<TT>Author: </TT><A href='?src=\ref[src];setauthor=1'>[scanner.cache.author ? scanner.cache.author : "*none*"]</A><BR>"
				dat += "<TT>Category: </TT><A href='?src=\ref[src];setcategory=1'>[scanner.cache.category ? scanner.cache.category : "*none*"]</A><BR>"
...
/obj/machinery/librarycomp/Topic(href, href_list)
	...
	if(href_list["setcategory"])
		...
		if(newcategory)
			scanner.cache.category = newcategory
	if(href_list["upload"])
		if(scanner)
			var/msg = server_db.upload_book(scanner.cache)
			if(msg)
				alert(msg)
			else
	...
	if(href_list["targetid"])
		if(bibledelay)
			...
			var/msg = server_db.download_book(href_list["targetid"], loc)
			if(msg)
				alert(msg)
			else
